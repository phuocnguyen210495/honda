<div class="w-full">
    <div class="sm:flex items-center w-full">
        <div class="sm:w-3/4">
            @if ($searchable = $hasSearchableColumns())
                <x-input first name="__" wire:model="query"
            ...
            <div class="flex flex-col shadow rounded-lg">
                <div class="-my-2 no-scrollbar overflow-x-auto">
                    <div class="py-2 align-middle inline-block min-w-full ">
                        <div class="overflow-hidden border-b border-gray-100 rounded-lg">
                            <table class="min-w-full divide-y divide-gray-100">
                                <thead>
                                @foreach($columns as $column)
                                    <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 text-gray-600 font-medium uppercase tracking-wider border-b border-gray-300">
                                        <a class="flex items-center" href=""
                                           wire:click.prevent="sortBy('{{ $column->modelColumn }}')">
                                            {{ $column->label }}
                                ...
                                @endforeach
                                @if ($hasActions())
                                    <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 text-gray-600 font-medium uppercase tracking-wider flex justify-end border-b border-gray-300">
                                        Actions
                                    </th>
                                @endif
                                </thead>
                                ...
                                @foreach($items as $item)
                                    <tr>
                                        @foreach($columns as $column)
                                            <td x-data="{text:'{{ $item->{$column->modelColumn} }}', open: false}"
                                                class="px-6 py-4 @if($column->truncate !== null) whitespace-nowrap @endif text-gray-700">

                                                <div class="flex items-center">
                                                    @if ($column->truncate !== null)
                                                        <p class="max-w-lg cursor-pointer" @click="open = !open"
                                                           x-text="open || text.length <= {{ $column->truncate }} ? text : text.substr(0, {{ $column->truncate }}) + '...'"></p>
                                                    @elseif($column->is('bool'))
                                                        <input aria-hidden="true" type="checkbox" disabled
                                                               class="form-checkbox rounded border-gray-300 text-{{ settings('color') }}-600 shadow-sm focus:border-{{ settings('color') }}-300"
                                                               @if ($item->{$column->modelColumn}) checked @endif />
                                        ...
                                        @endforeach
                                        @if ($hasActions())
                                            @foreach($__env->getShared()['_instance']->actions() as $k => $action)
                                                <td class="px-6 py-4 flex justify-end text-gray-700">
                                                    <x-dynamic-component :component="$action->component"
                                                                         wire:click="handleAction('{{ $k }}')"
