<div x-data="{@alpine($keys, $values, $selected, $multiple, $searchable), open: false, search: '', focused: false }" class="flex w-full flex-col-reverse" @search-input="search = $event.detail.value" @keydown="if (open && $event.key === 'Escape') { open = false }">
    ...
    <div class="mt-4">
        <button class="bg-white focus:outline-none focus:shadow-outline-blue rounded-lg text-gray-700 w-full flex items-center justify-between border" @click="open = !open" @focus="focused = true;" @blur="focused = false;">
        ...
        <div @click.away="open = false;" class="rounded-lg border bg-white mt-4 shadow-md" x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            <ul>
