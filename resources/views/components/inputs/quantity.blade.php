<div x-data="{@alpine($min, $max, $step, $value), removeIntervalId: -1, addIntervalId: -1 }"
    ...
    <div class="flex mt-2">
        <button @click="if (value > min + step) { value -= step }"
                ...
                class="bg-white rounded-lg px-4 placeholder-gray-700 font-display py-2.5 rounded-r-none border">
        ...
        <input x-model="value" type="text"
               ...
               class="bg-white rounded-lg px-4 border-t border-b placeholder-gray-700 font-display py-2.5 w-full rounded-l-none rounded-r-none focus:border-opacity-0 focus:outline-none"
               name="{{ $name }}" id="{{ $name }}">
        <button @click="if (value < max - step) { value = parseInt(value) + step }"
                ...
                class="bg-white rounded-lg px-4 placeholder-gray-700 font-display py-2.5 rounded-l-none border">
