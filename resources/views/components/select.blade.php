<div class="w-full @if (!$first) mt-4 @endif {{ $attributes->get('class') }}" {{ $attributes->except('class') }}>
    ...
    <div
        ...
        <div class="relative">
            ...
            <div @click.away="open = false;" class="rounded-lg border bg-white mt-4 shadow-md absolute w-full left-0" x-show="open"
                ...
                <ul>
                    <li class="p-4 pt-0 hidden sm:block" x-show="searchable">
                        <x-input name="search" first hideLabel :placeholder="__('Search an option')"/>
                    </li>
                    <div class="overflow-auto max-h-64 no-scrollbar focus:outline-none ">
                        <template x-for="(value, index) in values.filter(v => v.toString().includes(search))"
                                  :key="index">
                            <li @click="if (!multiple) { if (!selected.includes(value)) { selected = [value]; open = false } else { selected = []; open = false } } else { if (selected.includes(value)) { selected = selected.filter(e => e !== value) } else { selected.push(value) } }">
                                <button
                                    ...
                                    <x-icon name="check" x-show="selected.includes(value)" size="5"
                                            class="text-gray-500"/>
                                </button>
                            </li>
                        </template>
